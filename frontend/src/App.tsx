...
function App() {
  ...
  const montarCafe = useCallback(() => {
    ...
    fetch('/api/cafeteria/montar', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody) })
      .then(async res => {
        if (!res.ok) { const errorText = await res.text(); throw new Error(errorText || 'Erro na montagem.'); }
        return res.json();
      })
  ...
  const handleSelectBase = (id: number) => {
    ...
    else { setError('Máximo de 3 ingredientes base.'); setTimeout(() => setError(''), 3000); }
  ...
  const handleSelectAdicional = (id: number) => {
    ...
    else { setError('Máximo de 2 ingredientes adicionais.'); setTimeout(() => setError(''), 3000); }
  ...
  // NOVO: Função para o botão "Limpar Seleção" / "Começar de Novo"
  const handleReset = () => {
    setEtapa('BASE');
  ...
  const getStepClass = (stepName: Etapa) => {
      ...
      if ((etapa === 'ADICIONAIS' || etapa === 'RESUMO') && stepName === 'BASE') return 'completed';
      if ((etapa === 'RESUMO') && stepName === 'ADICIONAIS') return 'completed';
      return '';
  };
  
  const getConnectorClass = (stepNumber: number) => {
      if(stepNumber === 1 && (etapa === 'ADICIONAIS' || etapa === 'RESUMO' || etapa === 'SUCESSO')) return 'completed';
      if(stepNumber === 2 && (etapa === 'RESUMO' || etapa === 'SUCESSO')) return 'completed';
      return '';
  };
      ...
      return (
        <div className="success-message">
          ...
          <div className="action-buttons">
            <button className="confirmar-etapa" style={{width:'auto'}} onClick={handleReset}>Montar Outro Café</button>
          </div>
        </div>
      );
    }
      ...
      <>
        {etapa === 'BASE' && (
          <>
            ...
            <div className="ingrediente-lista">
              {ingredientesBase.map(ing => ( <button key={ing.id} className={selectedBaseIds.has(ing.id) ? 'selected' : ''} onClick={() => handleSelectBase(ing.id)} disabled={!selectedBaseIds.has(ing.id) && selectedBaseIds.size >= 3}>{ing.nome}</button> ))}
            </div>
            <div className="action-buttons"> <button className="confirmar-etapa" style={{width:'100%'}} onClick={avancarParaAdicionais} disabled={selectedBaseIds.size === 0}>Confirmar Base e ir para Adicionais</button> </div>
          </>
        )}
        ...
        {etapa === 'ADICIONAIS' && (
          <>
            ...
            <div className="ingrediente-lista">
              {ingredientesAdicionais.map(ing => ( <button key={ing.id} className={selectedAdicionalIds.has(ing.id) ? 'selected' : ''} onClick={() => handleSelectAdicional(ing.id)} disabled={!selectedAdicionalIds.has(ing.id) && selectedAdicionalIds.size >= 2}>{ing.nome}</button> ))}
            </div>
             <div className="action-buttons">
              <button className="voltar-etapa" onClick={() => setEtapa('BASE')}>Voltar</button>
              <button className="confirmar-etapa" onClick={() => setEtapa('RESUMO')}>Revisar Pedido</button>
            </div>
          </>
        )}
  ...
  return (
    <div className="container">
      <header><h1>Monte seu Café</h1><p>{textosDeEtapa[etapa]}</p></header>
      
      <div className="action-buttons" style={{justifyContent: 'flex-end', paddingBottom: '1rem'}}>
          {/* NOVO: Botão para Limpar Seleção / Começar de Novo */}
          {(etapa === 'BASE' || etapa === 'ADICIONAIS' || etapa === 'RESUMO') && 
            <button className="voltar-etapa" onClick={handleReset}>Limpar e Começar de Novo</button>
          }
      </div>
        ...
        <aside className="resumo">
          ...
          {isLoading ? <p>Atualizando...</p> : 
            ...
            cafeFinal ? (
              <div className="cafe-final-card">
                ...
                <p><strong>Base:</strong> {cafeFinal.ingredientesBase.join(', ')}</p>
                {cafeFinal.ingredientesAdicionais.length > 0 && ( <p><strong>Adicionais:</strong> {cafeFinal.ingredientesAdicionais.join(', ')}</p> )}
              </div>
          ...
          {etapa === 'RESUMO' && 
            <div className="action-buttons">
              ...
              <button className="confirmar-pedido" disabled={!cafeFinal || isLoading} onClick={handleConfirmarPedido}>Confirmar Pedido</button>
            </div>
          }
        </aside>
      </main>
